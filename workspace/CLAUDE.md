...
| **skill-ops** | `.claude/skills/skill-ops/SKILL.md` | Single entry point for the skill ecosystem — three routes: mine candidates / build·fix·audit one Skill (A→E flow, anatomy standard, 4-Tier audit) / sweep the whole portfolio (six metrics + upstream drift). Same folder: `references/` (four SOP checklists + QA), `templates/` (anatomy checklist + report formats), `scripts/` (entry-superset check), `human/` (bilingual visual explainers). Body is Traditional Chinese by design; English requests still trigger it. Replaces meta-skill. | Load when building/auditing/sweeping Skills, or when repeated work (N≥2) is worth packaging |
...
| **audio-transcribe** | `.claude/skills/audio-transcribe/SKILL.md` | Recording → text. You cannot hear audio; the **Antigravity CLI (`agy`)** listens for you — any language, speaker labels, an acoustic layer, four-layer output — then you verify and file it. Done = transcript **and** original audio side by side; routing the content onward is not this skill. **Requires `agy` installed and signed in** — SKILL §1 has the install line and a check. Whisper via Groq is an optional backup lane. Same folder: `references/QA.md` (what broke and why), `templates/` (prompt + correction block), `human/` (two pages to show the person). | Say "transcribe this" / drop an audio file path |
| **voiceink** | `.claude/skills/voiceink/SKILL.md` | Make VoiceInk — local Whisper dictation on macOS — come out in Traditional Chinese with Taiwan wording instead of Simplified. The agent walks the person through the clicks only they can make, then applies and verifies the zh-TW setup pack. Same folder: `references/QA.md` (pitfalls), `human/` (two pages to show the person). | Say "my dictation comes out Simplified" / "set up VoiceInk" |
| **eli5** | `.claude/skills/eli5/SKILL.md` | One sentence, deliberately: explain something to someone who knows nothing about it, as an HTML page with big pictures and few words. No steps, no checklist — expanding it is the change that breaks it. Same folder: `references/QA.md` (what people hit, why one sentence), `human/` (two pages to show the person). | Say "eli5" / "explain like I'm 5" / "dumb it down" |
| **explain1min** | `.claude/skills/explain1min/SKILL.md` | One-minute explainer diagram: a system or process as a self-contained HTML page — at most 3 flow diagrams, under 1000 words, and a decision diamond wherever the flow branches. Same folder: `templates/skeleton.html` (the layout to copy rather than start blank), `references/QA.md` (what people hit, why the rules are shaped that way), `human/` (two pages to show the person). Narrow on purpose: a single function or term gets an answer in words, not a diagram. | Say "explain how this works" / "draw me a diagram" |

...
| **audio-transcribe** | `.claude/skills/audio-transcribe/SKILL.md` | Recording → text. Opens with the hard limit — the agent cannot hear audio — then the one command that does the work, and the checks that stop a confident wrong transcript from being believed. Same folder: `references/QA.md` (what broke and why it is built this way), `human/` (two pages to show the person). | Say "transcribe this" / drop an audio file path |

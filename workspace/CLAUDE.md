...
| **audio-transcribe** | `.claude/skills/audio-transcribe/SKILL.md` | Recording → text. Opens with the hard limit — the agent cannot hear audio — then three routes: system dictation, a multimodal model in the browser, or a scripted setup. Same folder: `INSTALL.md` (Route 3 setup, written for you), `FLOW.md` (two diagrams — how it installs, how it runs), `VERIFY.md` (checks that produce evidence rather than claims). | Say "transcribe this" / drop an audio file path |

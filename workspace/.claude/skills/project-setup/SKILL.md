...
1. Search `` for existing related files (project name, keywords, people names)
2. If large amount of historical data found → **launch subagent for deep excavation**, don't do shallow scan
3. Scan `inbox/`, `projects/`, `refs/` for related discussion records
...
Add new project to `INDEX.md` "Active Projects" list:
```markdown
...
3. **Security check**
   - Reference `refs/security-checklist.md`
...
```
projects/{category}/{name}/debates/{YYYY-MM-DD}-{topic}/
├── transcript.md   ← Full conversation record
└── summary.md      ← Secretary summary (consensus + disagreements + ❓)
```

See `refs/debate-agents/debate-protocol.md` for details.

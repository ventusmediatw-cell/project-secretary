...
1. Copy scripts to your `.claude/scripts/` directory:
   ```bash
   mkdir -p .claude/scripts
   cp extras/claude-code/scripts/* .claude/scripts/
   chmod +x .claude/scripts/*.sh
   ```

2. Add the SessionStart hook to your `.claude/settings.local.json`:
   ```json
   {
     "hooks": {
       "SessionStart": [
   ...
   **Warning**: `settings.json.example` is a reference only. Merging into your existing settings is recommended — directly replacing your `.claude/settings.local.json` will overwrite your current configuration.
